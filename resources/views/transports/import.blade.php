...
<div style="padding: 20px;">
    <div style="background: white; border-radius: 8px; padding: 20px;">
        <h1>📤 Importar Transporte</h1>
        
        @if(session('error'))
            <div style="background: #fee2e2; color: #991b1b; padding: 12px; margin-bottom: 20px;">❌ {{ session('error') }}</div>
        @endif
        
        <form method="POST" action="{{ route('transports.import') }}" enctype="multipart/form-data">
            @csrf
            <div style="margin-bottom: 15px;">
                <label>Archivo Excel:</label>
                <input type="file" name="file" accept=".xlsx,.xls,.csv" required>
                <p style="font-size: 12px; color: #666;">Columnas: codigo, nombre, categoria, unidad, precio, termino, descripcion</p>
            </div>
