...
<div style="padding: 30px;">
    <div style="max-width: 1200px; margin: 0 auto; background: white; border-radius: 8px; padding: 20px;">
        ...
        <p style="margin-bottom: 24px;">Bienvenido, {{ Auth::user()->name ?? 'Usuario' }}!</p>
        ...
        <div style="background: #f3f4f6; padding: 16px; border-radius: 8px; margin-bottom: 24px;">
            ...
            <p><strong>Código:</strong> {{ $ultimoApu->code }}</p>
            <p><strong>Rubro:</strong> {{ Str::limit($ultimoApu->name, 80) }}</p>
            <p><strong>Costo Total:</strong> ${{ number_format($ultimoApu->total_cost ?? 0, 2) }}</p>
            <a href="/apu/{{ $ultimoApu->id }}" style="display: inline-block; margin-top: 8px; background: #3b82f6; color: white; padding: 4px 12px; border-radius: 4px; text-decoration: none;">Ver detalles →</a>
        </div>
        @endif
